...
export const drawGraphics = (
  ...
  switch (type) {
    case "RECTANGLE":
      Rectangle(g, child, imgsData, backgroundColor);
      break;
...
const drawRectangle = ({ g, x, y, width, height, backgroundColor }: any) => {
  g.clear();
  g.beginFill(backgroundColor);
...
const Rectangle = (
  ...
  if (!child.absoluteRenderBounds) {
    if (child.cornerRadius) {
      ...
      drawRectangle({ g, x, y, width, height, backgroundColor });
    }
    return;
  }
  if (!child.fills || child.fills.length === 0 || !child.fills[0].color) return;
